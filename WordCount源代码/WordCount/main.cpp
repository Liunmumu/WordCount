...
void	open_file(char *filename);											//	打开文件	预先判断文件是否存在
...
int main(int argc, char *argv[]){
	if (strcmp(argv[1], "-s") && strcmp(argv[1],"-x")){			//	parameter 非 "-s" "-x"
		open_file(argv[2]);										//	预先判断文件是否存在 否则直接退出
	}
...
int		count_of_character(char *filename){				//	统计字符个数
	...
	int count = file.tellg();							//	读取字符数		字符'\n'算两个 因此需要 减去行数 加1
	cout << filename << " 文件的 字符数为 " << count - count_of_line(filename,false) + 1 << endl;
